...
- **WHEN** the top face has a triangle at position 0 (size 1) and `flip` is called
...
- **WHEN** the top face has a kite at position 0 (size 2) and `flip` is called
...
- **WHEN** the bottom face has right-half blocks and `flip` is called
...
- **WHEN** a face has blocks at positions ≥ 6 and `flip` is called
...
#### Scenario: Source Cube unchanged after flip
- **WHEN** `flip` is called on a Cube
